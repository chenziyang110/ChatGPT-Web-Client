...
- `waiting_user` 时工具持续等待用户选择；`uncertain` 时只读核验原问题并等待答案，无法核验再请用户处理，不能重发、自动接管或确认恢复。
...
- **处理好了，继续 / 再试一次**：重试同一条尚未发送的任务，保留目标、问题和请求键。
- **取消任务**：取消尚未发送的任务。
- **已核对，结束此任务**：仅适用于消息可能已发送的情况；不会重发。后续队列保持暂停，可另行继续。

接管也支持应用内快捷键，可在设置中配置；组合不显示在工作区。接管不会撤回已发送消息，也不保证网页停止生成。Agent 只需查询同一任务，并告知人类何时需要选择。
