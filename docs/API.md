...
The monitor reads each live supported page every 1.5 seconds, with at most one outstanding read per page. Completion requires a recognized finished assistant turn with stable content for three seconds and no busy indicator. The task executor and monitor share a completion fingerprint, so they cannot double-count one reply or recreate a notification after it was handled. No remote-page bridge is added. This observes live client pages, not all remote ChatGPT activity. Navigating away or safely hibernating an idle background page clears observed running status without fabricating completion.
