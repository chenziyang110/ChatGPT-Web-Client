...
export const agentHelp = {
  ...
  cli: {
    ...
    waiting: 'ask is one blocking process with no local deadline by default. Keep that same process through pending, running and waiting_user. Do not create a periodic monitor/goal or poll tasks.get. If a process/session ID is returned, use the longest available wait on that same process. Saved requests reconnect automatically.',
    streaming: '--stream is opt-in for displaying live text and heartbeats; omit it when the Agent only needs the final answer. Only done confirms success and contains result.response. stderr contains recovery information, never token.',
  ...
  wait: { method: 'tasks.wait', params: { id: 'TASK_ID', timeoutMs: 25000, afterUpdatedAt: 'LAST_TASK_UPDATED_AT_NUMBER', updates: true } },
  ...
  response: 'HTTP returns {ok:true,result:task} or {ok:false,error:string}. Prefer the blocking CLI. HTTP-only callers repeat tasks.wait long-polls for the same task and must not poll tasks.get every 1–2 seconds.',
  states: {
    pending: 'Queued. Keep the same blocking wait. If paused, report queues.status to the human; do not resume it yourself.',
    running: 'The client controls the page; the human can view a live read-only preview.',
    waiting_user: 'Notify the human once and keep the same blocking process. After takeover, the human fixes the page and clicks 交还 Agent 并继续; keep waiting for the same task and do not mark it blocked or submit another copy.',
  ...
  concurrency: 'Different conversations, including those in one account, use independent pages and can wait for replies concurrently. Each conversation is FIFO. The global limit of two applies only to active browser operations; idle waits and confirmed sent replies release execution capacity but retain their conversation locks. A new conversation does not wait for an unrelated reply. Use workspace.status.pages to inspect tabs. queues.status includes account summaries and conversation-specific rows. Never take over or resume another conversation.',
...
<pre>chatgpt-agent.exe ask --account ACCOUNT_ID --new --text-file question.txt
chatgpt-agent.exe resume TASK_ID</pre>
<p>工具负责保存请求、静默等待、断线重连和取回完整答案。默认不要加 --stream，也不要另建轮询任务；如果 Agent 工具返回进程仍在运行，请对同一进程使用最长等待。只有 done 表示完成。</p>
...
<h2>2. 提问与等待</h2><p>先保存问题、完整请求和新 UUID，再调用 tasks.create。保存返回的任务 ID 和 conversationId，用 tasks.wait 长轮询同一任务。done 后读取 result.response。重试同一个问题必须保持请求键和参数完全一致。</p>
<h2>3. 等待与恢复</h2><p>waiting_user 时只通知用户一次并保持原进程；用户处理后点击「交还 Agent 并继续」，工具会自动续等同一任务。uncertain 时只读核验原问题并等待回答。不要把人工等待标记为 blocked，不要重复提交或擅自恢复队列。</p>
