...
目前支持普通个人会话，列表来自本地登记和访问记录。项目、自定义 GPT、临时会话及完整云端历史同步不在本阶段范围内。同一会话的任务按队列串行；不同账号、不同会话可独立等待回复。全局最多两个网页操作同时执行，等待上一轮回复和已确认发送后的等待不占执行名额；准备写入前重新取得名额并核对页面。正在回复的网页保留，后台约每两秒检查一次。
