...
export function taskAttention(error: string, sent = false, takeover = false): TaskAttention {
  const entry: Pick<TaskAttention, 'kind' | 'title' | 'detail'> = sent ? {
    ...
    : /COMPOSER_NOT_READY|SEND_UNAVAILABLE|PAGE_UNSUPPORTED|PAGE_CHANGED|TARGET_CHANGED|PAGE_SCRIPT_FAILED|Element unavailable|Element is not editable/.test(error) ? { kind: 'page', title: '此任务上次未能操作目标页面', detail: '这是自动任务上次执行时的检查结果，不代表当前网页不能手动操作。问题尚未发送。可以重试；若目标页面有草稿，请先接管处理。' }
